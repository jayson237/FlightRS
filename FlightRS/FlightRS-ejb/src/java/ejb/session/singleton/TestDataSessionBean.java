...
public class TestDataSessionBean {
    ...
    private void doDataInit() {
        try {
            ...
            List<CabinClass> list = new ArrayList<>();
            list.add(cabin);
            aircraftConfigurationSessionBean.createNewAircraftConfig(config, list);

            list.clear();
            cabin = new CabinClass(CabinClassType.F, 1, 5, 2, "1-1", 10);
            list.add(cabin);
            cabin = new CabinClass(CabinClassType.J, 1, 5, 4, "2-2", 20);
            list.add(cabin);
            cabin = new CabinClass(CabinClassType.Y, 1, 25, 6, "3-3", 150);
            list.add(cabin);
            config = new AircraftConfiguration("BOEING 737 THREE CLASSES", 3, 180);
            aircraftConfigurationSessionBean.createNewAircraftConfig(config, list);

            list.clear();
            cabin = new CabinClass(CabinClassType.Y, 2, 38, 10, "3-4-3", 380);
            list.add(cabin);
            config = new AircraftConfiguration("BOEING 747 ALL ECONOMY", 1, 380);
            aircraftConfigurationSessionBean.createNewAircraftConfig(config, list);

            list.clear();
            cabin = new CabinClass(CabinClassType.F, 1, 5, 2, "1-1", 10);
            list.add(cabin);
            cabin = new CabinClass(CabinClassType.J, 2, 5, 6, "2-2-2", 30);
            list.add(cabin);
            cabin = new CabinClass(CabinClassType.Y, 2, 32, 10, "3-4-3", 320);
            list.add(cabin);
            config = new AircraftConfiguration("BOEING 747 THREE CLASSES", 3, 360);
            aircraftConfigurationSessionBean.createNewAircraftConfig(config, list);
            ...
            findFlight411 = flightSessionBean.retrieveFlightByNumber("ML412");
            returnPlan411.setFlight(findFlight411);
            Calendar c = Calendar.getInstance();
            c.setTime(outBoundInfo411.getKey());
            double duration = outBoundInfo411.getValue();
            int hour = (int) duration;
            int min = (int) (duration % 1 * 60);
            c.add(Calendar.HOUR_OF_DAY, hour);
            c.add(Calendar.MINUTE, min);
            ...
            Date newDeparture = c.getTime();
            Pair<Date, Double> returnInfoPair = new Pair<>(newDeparture, outBoundInfo411.getValue());
            flightSchedulePlanSessionBean.createNewReturnFlightSchedulePlan(returnPlan411, newPlan411, findFlight411.getFlightId(), returnInfoPair, 2);

            List<Fare> fares511 = new ArrayList<>();
            fares511.add(new Fare(BigDecimal.valueOf(3100), CabinClassType.F));
            fares511.add(new Fare(BigDecimal.valueOf(1600), CabinClassType.F));
            fares511.add(new Fare(BigDecimal.valueOf(600), CabinClassType.F));
